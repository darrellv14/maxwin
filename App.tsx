        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold tracking-tight text-white font-mono">
              PROFIT<span className="text-profit-green">HUNTER</span><span className="text-gray-600 text-sm ml-1">v2.1</span>
            </h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6">
          ...
          <div className="col-span-12 md:col-span-3 space-y-4">
            <div className="bg-terminal-gray border border-gray-800 p-4 rounded-lg">
                <label className="block text-xs font-mono text-gray-500 mb-1">ASSET TICKER</label>
                <div className="relative">
                  ...
                  />
                  <div className="absolute right-3 top-2 text-xs text-gray-600">JKSE</div>
                </div>
            </div>
            ...
            <div className="bg-terminal-gray border border-gray-800 p-4 rounded-lg space-y-3">
                <div className="flex justify-between items-center">
                    ...
                    <span className={`text-sm font-bold font-mono ${
                        (current?.rsi || 50) > 70 ? 'text-loss-red' : (current?.rsi || 50) < 30 ? 'text-profit-green' : 'text-gray-300'
                    }`}>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gray-800">
                    ...
                    <span className={`text-sm font-bold font-mono ${
                        (current?.close || 0) > (current?.sma50 || 0) ? 'text-profit-green' : 'text-loss-red'
                    }`}>
          ...
          <div className="col-span-12 md:col-span-9 space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               <StatCard 
                 label="Last Price" 
                 value={current?.close.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }).replace('Rp', '') || '0'} 
               ...
               <StatCard 
                 ...
                 color={analysis?.signal === 'BUY' ? 'text-profit-green' : analysis?.signal === 'SELL' ? 'text-loss-red' : 'text-gray-500'}
               />
            </div>
                        ...
                        <tbody className="text-gray-300">
                            {[...data].reverse().slice(0, 8).map((d, i) => (
                                <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/50 transition-colors">
                                    ...
                                    <td className={`py-2 ${(d.rsi || 50) > 70 ? 'text-red-400' : (d.rsi || 50) < 30 ? 'text-green-400' : ''}`}>
                                        {d.rsi?.toFixed(1)}
                                    </td>
                                    <td className="py-2 text-right text-gray-400">{d.volume.toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
